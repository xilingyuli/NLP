...
public class WordSegmentation {
    ...
    public static String divideArticle(String str){
        ...
        allWords.forEach(NameIdentification::nameIdentify);
    ...
    public static void run(String path){
        ...
        for(File f:files){
            if(f.isFile()&&f.getName().endsWith(".txt")){
                ...
                try {
                    PrintStream ps=new PrintStream(new FileOutputStream(f.getName().replace(".txt","")+"分词结果.txt"));
                    System.setOut(ps);
                }catch (Exception e){
                    e.printStackTrace();
                }
